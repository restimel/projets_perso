...
//permet d'ajouter la valeur d'une plaque/operateur à la formule de calcul
...
Nombre.prototype.createCalcul = function(){
	...
	if(this.valeur == jeuCible.value){
		...
		calcul.addEventListener("click",function(){document.getElementById("entreeFormule").value += that.valeur;});
	}
	this.refCalcul.appendChild(del);
	this.refCalcul.appendChild(calcul);
...
function createWorker(){
	try{
		...
		//la création du worker a échoué (sans doute un problème de same origin policy),
		//on passe en solution de contournement
		useScript();
	}
	return w;
}

//utilisation du script fallback sans worker
function useScript(){
	var script = document.createElement("script");
		script.src="./solution.js";
		script.onload=function(){document.body.removeChild(script);};//retrait de l'élément HTML du DOM
...
window.addEventListener("load",function(){ //initialisation
	//attachements des événements aux éléments
	document.getElementById("bStart").addEventListener("click",initialisation,false);
	document.getElementById("bRestart").addEventListener("click",initialisation,false);
	var liste=document.querySelectorAll("#operations button"),i=liste.length;
	while(i--){
